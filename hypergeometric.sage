...
class PochhammerSymbol():
    ...
    def _latex_(self):
        ...
        """
        return "(%s)_{%s}" % (','.join(map(str, self.list_a)), str(self.n))
    ...
    def evaluate(self):
        ...
        """
        if self.n >= 0:
            return prod([ self._eval_single(elm) for elm in self.list_a ])
        else:
            return 0

def poch(a, n): 
    """
    Evauluate Pochhammer symbols symbolically.
...
class HypergeometricSeries():
    ...
    """ 
    def __init__(self, list_a, list_b, z):
        """
    ...
    def _latex_(self):
        ...
        """
        return "{}_{%i}F_{%i}\\biggl[\genfrac..{0pt}{}{%s}{%s};%s\\biggr]" \
            % (len(self.list_a), len(self.list_b), \
            ','.join(map(str, self.list_a)), \
            ','.join(map(str, self.list_b)), str(self.z))
    ...
    def __repr__(self):
        ...
        """
        return "(%i)_F_(%i)(%s;%s;%s)" % (len(self.list_a), len(self.list_b), \
            ','.join(map(str, self.list_a)), \
            ','.join(map(str, self.list_b)), str(self.z))
...
def hs(nom, denom, z):
    """
    Evaluate hypergeometric series symbolically.
